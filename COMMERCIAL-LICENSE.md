# Qoomb - Commercial Licensing

Copyright © 2026 Benjamin Gröner

---

## Overview

Qoomb is licensed under the **Fair Source License v1.0** with a **10-employee threshold**.

If your use falls outside the [permitted free use](LICENSE.md), you need a commercial license from Benjamin Gröner.

---

## When Do You Need a Commercial License?

You need a commercial license if you want to use Qoomb as:

### 1. Large Organization (≥ 10 Employees)

- Your organization has 10 or more employees (full-time, part-time, contractors)
- You want to use Qoomb internally for your company
- **Example:** A company with 50 employees wants to use Qoomb as an internal hive platform
- **Note:** This threshold applies to **organizations only**. Families of any size are exempt — see the Family Use free tier below.

### 2. Hosted Service (SaaS)

- You want to offer Qoomb as a service to third parties
- You operate a Qoomb instance for your customers
- **Example:** A hosting provider wants to sell Qoomb as "Hive-as-a-Service"

### 3. Redistribution

- You want to sell Qoomb as part of a commercial product
- You want to integrate Qoomb into your commercial software suite
- **Example:** A software vendor wants to bundle Qoomb as a module in their enterprise suite

---

## What's Free Under the Fair Source License?

The following uses are **free** and **do not require a commercial license**:

✅ **Personal Use** - Use for personal, non-commercial purposes
✅ **Development** - Development, testing, evaluation
✅ **Small Organizations** - Organizations with **fewer than 10 employees**
✅ **Non-Profit** - Non-profit organizations with < 10 employees
✅ **Education** - Educational and research purposes
✅ **Families** - Families of **any size** for strictly private, non-commercial use (no money charged, no advertising, no services to third parties)

See [LICENSE.md](LICENSE.md) for complete details.

---

## Commercial License Models

Benjamin Gröner offers flexible commercial license models tailored to your needs:

### Model 1: Perpetual License

- **One-time payment** for a specific version
- Unlimited use of the licensed version
- Optional: Maintenance and update contract (annual)
- **Suitable for:** Companies seeking long-term stability

### Model 2: Subscription License

- **Annual or monthly payment**
- Access to all updates and new versions
- Technical support included
- **Suitable for:** Companies wanting the latest version always

### Model 3: SaaS/Hosting License

- **Revenue sharing** or **pay-per-user**
- Specifically for hosting providers and SaaS platforms
- Flexible scaling with your business
- **Suitable for:** Service providers offering Qoomb to their customers

### Model 4: OEM/Embedding License

- **Individually negotiated** for redistribution
- White-label options available
- Technical integration support
- **Suitable for:** Software vendors embedding Qoomb

---

## Pricing

Prices for commercial licenses are **individually negotiated** based on:

- **Organization size** (number of employees)
- **Number of users** (for SaaS/Hosting)
- **Type of use** (internal, SaaS, OEM)
- **Support level** (Community, Business, Enterprise)
- **Geographic region**

**Transparency Principle:** We offer fair, reasonable prices aligned with your usage.

---

## Contact for Commercial Licensing

If you need a commercial license, please contact:

### Benjamin Gröner

📧 **Email:** <bgroener@coqoon.com>
🌐 **Web:** <https://github.com/COQOON-labs/qoomb>
💬 **GitHub Discussions:** <https://github.com/COQOON-labs/qoomb/discussions>

**Please provide:**

1. Your company/organization
2. Number of employees
3. Intended use (internal, SaaS, OEM, etc.)
4. Estimated number of end users
5. Preferred license model (if known)

**Response time:** We typically respond within 2 business days.

---

## Frequently Asked Questions (FAQ)

### Q: I currently have 9 employees. What happens when I reach 10?

**A:** You have a **90-day grace period** from the time you reach 10 employees to obtain a commercial license. We understand companies grow, and we want to make the transition smooth.

### Q: I run Qoomb for my family. Do I need a license?

**A:** **No** — families are exempt regardless of size, as long as the use is strictly private and non-commercial: no money charged, no advertising revenue, and no services provided to third parties. A family of 20 people is treated the same as a family of 4.

### Q: We are a large family with 15 members. Does the 10-employee threshold apply to us?

**A:** **No.** The 10-employee threshold applies to **organizations** (companies, associations, businesses). Families are explicitly exempt from this limit. You can use Qoomb for free as long as it is for your family's own private, non-commercial use.

### Q: I'm a freelancer using Qoomb for my projects. Do I need a license?

**A:** **No**, as long as you have fewer than 10 employees (including contractors), use is free.

### Q: I want to modify Qoomb and offer it to my customers. Is that allowed?

**A:** That's **redistribution** or **hosted service** and requires a commercial license. Please contact us for a custom quote.

### Q: Can I fork Qoomb and develop it myself?

**A:** **Yes**, you may fork, modify, and develop Qoomb, as long as you comply with the Fair Source License conditions (i.e., no commercial use outside the limits without a license).

### Q: Do you offer discounts for non-profits with ≥ 10 employees?

**A:** **Yes**, we offer significant discounts (often 50-80%) for non-profit organizations. Please contact us with details about your organization.

### Q: Can I get a trial version for my large company?

**A:** **Yes**, we offer a **90-day evaluation license** for large organizations. Please contact us.

### Q: Are there academic licenses for universities?

**A:** **Educational use is free** under the Fair Source License, regardless of institution size. For administrative use (not research/teaching), please contact us.

---

## Technical Support

### Community Support (free)

- GitHub Issues
- GitHub Discussions
- Community Forum

### Business Support (commercial license)

- Email support (48h response time)
- Bug fixes for your version
- Security updates

### Enterprise Support (premium)

- Dedicated support contact
- 24h response time
- Prioritized bug fixes
- Phone support
- Architecture consulting

---

## Compliance and Audit

Benjamin Gröner reserves the right to verify compliance with license terms. If you have a commercial license, you agree to:

- **Annual usage reports** (number of users, employees)
- **Audit cooperation** (on request, max 1x per year)
- **Immediate notification** if license terms are exceeded

**Transparency Promise:** We conduct audits fairly and cooperatively, aiming for win-win solutions, not penalties.

---

## Changes to These License Terms

These commercial license terms may change. Changes apply:

- **Immediately** for new licenses
- **Upon renewal** for existing subscription licenses
- **Not retroactively** for perpetual licenses (already purchased versions remain under old terms)

---

## Legal

**Jurisdiction:** Germany
**Applicable Law:** German Law
**Language:** In case of discrepancies between German and English versions, the English version prevails.

---

**Qoomb** - Privacy-first, self-hosted hive organization platform
**Copyright © 2026 Benjamin Gröner**
**License:** Fair Source License v1.0 (10-User Limit)

For technical documentation and installation see [README.md](README.md)
