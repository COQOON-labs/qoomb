# Architecture Decision Records (ADRs)

This directory contains architecture decision records for the Qoomb project.
We use [MADR](https://adr.github.io/madr/) (Markdown Architectural Decision Records) format.

## Index

| ADR                                                | Title                                        | Status     | Date       |
| -------------------------------------------------- | -------------------------------------------- | ---------- | ---------- |
| [ADR-0001](0001-adr-process.md)                    | Use MADR for architecture decisions          | Accepted   | 2026-02-20 |
| [ADR-0002](0002-shared-domain-utilities.md)        | Domain-driven code structure                 | Accepted   | 2026-02-20 |
| [ADR-0003](0003-presentation-hooks-pattern.md)     | Presentation hooks for derived UI state      | Deprecated | 2026-02-20 |
| [ADR-0004](0004-cloud-agnostic-architecture.md)    | Cloud-agnostic architecture & portable stack | Accepted   | 2026-02-20 |
| [ADR-0005](0005-hybrid-encryption-architecture.md) | Hybrid encryption architecture               | Accepted   | 2026-02-20 |

## Format

Each ADR follows the MADR template:

- **Status** — Proposed, Accepted, Deprecated, Superseded
- **Context** — What is the issue? What forces are at play?
- **Decision** — What is the change being proposed/made?
- **Consequences** — What becomes easier or harder?
